 ...
 * 
 * <p>Tests trade-specific extraction logic including CSV parsing delegation
 * and conversion of raw trade records to standardized data models.</p>
 */
public class XbondTradeExtractorTest {
    ...
    @Test
    public void testGetCategory_ReturnsTradeData() {
        assertEquals("Category should be 'TradeData'", "TradeData", extractor.getCategory());
    }
    ...
    public void testCreateCosClient_ReturnsCosClientImpl() throws ETLException {
        ...
        // We can trust it returns a working client; integration tests verify actual behavior
    }
    ...
    public void testParseCsvFile_PropagatesETLExceptionFromParser() throws ETLException {
        // Setup
        File csvFile = new File("test.csv");
        when(mockTradeCsvParser.parse(csvFile)).thenThrow(new ETLException("CSV_PARSE", csvFile.getName(), "Parser error"));
    ...
    public void testConvertRawRecords_SingleValidRecord_CreatesModel() throws ETLException {
        ...
        assertEquals("counterparty should match", "C001", model.getCounterparty());
        assertEquals("tradeId should match", "T20250101-001", model.getTradeId());
        assertEquals("eventTime should match transactTime", LocalDateTime.of(2025, 1, 1, 10, 30, 0), model.getEventTime());
        assertEquals("receiveTime should match recvTime", LocalDateTime.of(2025, 1, 1, 10, 30, 5), model.getReceiveTime());
    ...
    @Test
    public void testConvertRawRecords_MultipleRecordsSameMqOffset_CreatesOneModel() throws ETLException {
        // Setup: two records with same mqOffset (should be grouped together)
        List<RawTradeRecord> rawRecords = new ArrayList<>();
        ...
        RawTradeRecord record2 = new RawTradeRecord(
            2L, "1021001", 1, 100.6, 2.6, "YTM", 2000L, "C002", "T002",
            LocalDateTime.of(2025, 1, 1, 10, 31, 0), 500L, // Same mqOffset
        ...
        // Verify: only one model created (grouped by mqOffset)
        assertEquals("Should create one model for same mqOffset", 1, result.size());
        
        XbondTradeDataModel model = (XbondTradeDataModel) result.get(0);
        // The second record's values may overwrite the first's in the mapping
        // This is expected behavior: last record in group determines field values
        // (though in practice each mqOffset should have only one trade)
        assertEquals("exchProductId should have .IB suffix", "1021001.IB", model.getExchProductId());
        // settleSpeed from last record
        assertEquals("settleSpeed should be 1", Integer.valueOf(1), model.getSettleSpeed());
        // tradePrice from last record (100.6)
        assertEquals("tradePrice should be from last record", Double.valueOf(100.6), model.getTradePrice());
        assertEquals("tradeYield should be from last record", Double.valueOf(2.6), model.getTradeYield());
        assertEquals("tradeYieldType should be YTM", "YTM", model.getTradeYieldType());
        assertEquals("tradeVolume should be from last record", Long.valueOf(2000), model.getTradeVolume());
        assertEquals("counterparty should be from last record", "C002", model.getCounterparty());
        assertEquals("tradeId should be from last record", "T002", model.getTradeId());
        
        // Timestamps should be from the first record (since we use firstRecord for eventTime/receiveTime)
        // Actually our implementation uses firstRecord for eventTime and receiveTime
        assertEquals("eventTime should be from first record", LocalDateTime.of(2025, 1, 1, 10, 30, 0), model.getEventTime());
        assertEquals("receiveTime should be from first record", LocalDateTime.of(2025, 1, 1, 10, 30, 5), model.getReceiveTime());
        
        assertTrue("Model should be valid", model.validate());
    }
    ...
    @Test
    public void testConvertRawRecords_InvalidRecordInGroup_SkipsInvalidModel() throws ETLException {
        // Setup: one valid record and one invalid record with same mqOffset
        List<RawTradeRecord> rawRecords = new ArrayList<>();
        ...
        RawTradeRecord invalidRecord = new RawTradeRecord(
            ...
            LocalDateTime.of(2025, 1, 1, 10, 35, 0), 500L, // Same mqOffset
            LocalDateTime.of(2025, 1, 1, 10, 35, 5)
        );
        ...
        // Verify: no models created because group contains invalid record
        // Our implementation validates the final model after processing all records in group
        // If the model fails validation, it's skipped
        // Since the invalid record overwrites fields with empty tradeId, model validation fails
        assertTrue("Should skip invalid model", result.isEmpty());
    }
    
    @Test
    public void testConvertRawRecords_RecordWithoutMqOffset_Skipped() throws ETLException {
        // Setup: record with null mqOffset (should be skipped during grouping)
        List<RawTradeRecord> rawRecords = new ArrayList<>();
        ...
        // Verify: no models created because record with null mqOffset is filtered out
        assertTrue("Records with null mqOffset should be skipped", result.isEmpty());
    }
    ...
    public void testExtract_SetsCurrentBusinessDate() throws ETLException {
        ...
        when(mockFile.getKey()).thenReturn("TradeData/2025-01-01/file1.csv");
