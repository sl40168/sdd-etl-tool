...
public class DolphinDBLoader implements Loader {
    ...
    /**
     * Groups data by dataType for batch loading.
     */
    private Map<String, List<TargetDataModel>> groupByDataType(List<? extends TargetDataModel> data) {
        Map<String, List<TargetDataModel>> grouped = new HashMap<>();

        for (TargetDataModel model : data) {
            String dataType = model.getDataType();
            grouped.computeIfAbsent(dataType, k -> new ArrayList<>()).add(model);
        }
    ...
    /**
     * Loads a group of same-type models to a table.
     */
    private void loadGroupToTable(List<TargetDataModel> group, String tableName) throws LoaderException {
        if (group.isEmpty()) {
            return;
        }
        ...
        try {
            ...
            // Convert to column-based format
            Map<String, Object> columns = DataConverter.convertToColumns(group);
            ...
            if (!args.isEmpty()) {
                ...
                java.lang.reflect.Method method = conn.getClass().getMethod("run", String.class);
                method.invoke(conn, insertSql.toString());
                logger.debug("Inserted {} records to table {}", group.size(), tableName);
            }
        } catch (Exception e) {
            throw new DataLoadingException("Failed to load data to table " + tableName + ": " + e.getMessage(), e);
        }
